...
public class UniGraphVertexStep<E extends Element> extends AbstractStep<Vertex, E> implements ReceivesPredicatesHolder<Vertex, E>, PropertyFetcher {
    ...
    private Iterator<Traverser.Admin<E>> getTraversersWithProperties(Iterator<Traverser.Admin<E>> traversers) {
        ...
        List<DeferredVertex> deferredVertices = copyTraversers.stream().map(traverser -> ((DeferredVertex) traverser.get())).collect(Collectors.toList());
        DeferredVertexQuery query = new DeferredVertexQuery(deferredVertices, propertyKeys, this.stepDescriptor);
        defferedVertexControllers.stream().forEach(controller -> controller.fetchProperties(query));
