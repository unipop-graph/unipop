...
public class UniGraphPropertiesSideEffectStep<S extends BaseElement> extends AbstractStep<S, S> {
    ...
    public UniGraphPropertiesSideEffectStep(Traversal.Admin traversal, ControllerProvider controllerProvider) {
        super(traversal);
    ...
    private Iterator<Traverser<S>> query(ExpandableStepIterator<S> traversers) {
        ...
        if (!elements.isEmpty())
            results.addResults((Iterator<S>) controllerProvider.properties(elements).iterator());
